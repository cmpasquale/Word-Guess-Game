...
function gameTime () {
  ...
  loses = [];
    for (var i = 0; i < letterSpaces.length; i++){
      emptyandGuessed.push("_");
    }
  document.getElementById("remaining").innerHTML = guesses;
  document.getElementById("slots").innerHTML = emptyandGuessed.join(" ");
      ...
      if (letterPresent){
        ...
        for (var a = 0; a < letterSpaces.length; a++){
            ...
            if (cpuPick[a] === correctLetter){
                emptyandGuessed [a] = correctLetter;
            };
        };
      }
        else {
...
function endRound(){
  ...
  document.getElementById("slots").innerHTML = emptyandGuessed.join(" ");
  document.getElementById("loses").innerHTML = loses.join(" ");
  if (letterNumber.toString() === emptyandGuessed.toString() ){
    wins ++
