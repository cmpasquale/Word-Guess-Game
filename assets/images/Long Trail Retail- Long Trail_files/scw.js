...
//
//   Added Language French and Italian by Massimo Balestra e-Quantum inc.
//            January 2016
//
...
//            for a full explanation.scwSource 
//
...
//******************************************************************************
    ...
    // -----------------------------------------------------------------------------
    //Functions added by John Freeman May 13th 2016
    //------------------------------------------------------------------------------
    function scw_SetZIndex(zindex)
    {
      var iframeSCW = document.getElementById('scwIframe');
      if (iframeSCW)
      {
        iframeSCW.style.zIndex = zindex;
      }
      var tableSCW = document.getElementById('scw');
      if (tableSCW)
      {
        tableSCW.style.zIndex = zindex;
      }
    }
    ...
    function scwSetDefaultLanguage()   { 
      try {
        ...
        scwArrFullMonthNames   = ['January','February','March','April','May','June',
                                 'July','August','September','October','November','December'];
        scwArrWeekDays         = ['Sunday','Monday','Tuesday','Wednesday',
    ...
    function date2Eng(dte) {
      if (scwLanguage == 'en') {
        return dte;
      } else {
        scwSetLanguage('en');
        var monEn = scwArrMonthNames;
        scwSetLanguage(scwLanguage);
        var dmy = dte.split(/ /);
        return (monEn[scwArrMonthNames.indexOf(dmy[1])]+'-'+dmy[0]+'-'+dmy[2]);
      }
    }
    function scwSetLanguage(lang) {
      if (lang == 'it') {   // Italiano
        scwToday               = 'Oggi:';
        scwClear               = 'Svuota';
        scwDrag                = 'Clicca e trascina';
        scwArrMonthNames       = ['gen','feb','mar','apr','mag','giu',
                                 'lug','ago','Set','ott','nov','dic'];
        scwArrFullMonthNames   = ['gennaio','febbraio','marzo','aprile','magio','giugno',
                                 'luglio','agosto','settembre','ottobre','novembre','dicembre'];
        scwArrWeekDays         = ['domenica','lunedì','martedì','mercoledì',
                                 'giovedì','venerdì','sabato'];
        scwArrWeekInits        = ['D','L','M','M','G','V','S'];
        scwInvalidDateMsg      = 'Data non valida.\n';
        scwOutOfRangeMsg       = 'La data è fuori dell\'intervallo consentito.';
        scwDoesNotExistMsg     = 'La data non esiste.';
        scwInvalidAlert        = ['Data non valida (',') ignorata.'];
        scwDateDisablingError  = ['Errore ',' L\'oggetto non è una data.'];
        scwRangeDisablingError = ['Errore ',
                                 ' dovrebbe avere due elementi.'];
        if (newDateFormat != 1)
        {
          newDateFormat = 1;
          scwDateDisplayFormat = '##/%%/^^';    // 'DD-MM-YY'
          scwDateOutputFormat = '## %%% ^^^^';  // 'DD-MMM-YYYY'
        }
      } else if (lang == 'fr') {   // Français
        scwToday               = 'aujourd\'hui:';
        scwClear               = 'nettoyer';
        scwDrag                = 'Cliquez ici pour glisser';
        scwArrMonthNames       = ['jan','feb','mar','avr','mai','jui',
                                 'jui','aoû','sep','oct','nov','déc'];
        scwArrFullMonthNames   = ['janvier','février','mars','avril','mai','juin',
                                 'juillet','août','septembre','octobre','novembre','décembre'];
        scwArrWeekDays         = ['dimanche', 'lundi','mardi','mercredi',
                                 'jeudi','vendredi','samedi'];
        scwArrWeekInits        = ['D','L','M','M','J','V','S'];
        scwInvalidDateMsg      = 'La date indiquée est pas valide.\n';
        scwOutOfRangeMsg       = 'La date indiquée est hors des limites.';
        scwDoesNotExistMsg     = 'La date indiquée ne existe pas.';
        scwInvalidAlert        = ['date invalide (',') ignoré.'];
        scwDateDisablingError  = ['Erreur ',' est pas un objet Date.'];
        scwRangeDisablingError = ['Erreur ',
                                 ' devrait être composé de deux éléments.'];
        if (newDateFormat != 1)
        {
          newDateFormat = 1;
          scwDateDisplayFormat = '##/%%/^^';    // 'DD-MM-YY'
        ...
        {
          newDateFormat = 1;
          scwDateDisplayFormat = '%%-##-^^';     // e.g. 'MMM-DD-YYYY' for the US
          scwDateOutputFormat = '%%%-##-^^^^';
        }
      }
    }
    ...
    //     Added  Aug 14 2013 by Massimo Balestra
    // O            date of the month as ordinal number (1st, 2nd, ...)
    // E            day of the wEek
    // MMMM            Full name of the Month

    //            NEW FORMAT
    // #            date in the month without zero filling
    // ##           date in the month left zero filled
    // %            month number without zero filling
    // %%           month number left zero filled
    // %%%          month string from scwArrMonthNames
    // ^^           year number in two digits
    // ^^^^         year number in four digits
    //     Added  Aug 14 2013 by Massimo Balestra
    // {            date of the month as ordinal number (1st, 2nd, ...)
    // &            day of the week
    ...
    // Ouput date case options
    // 0 = None
    // 1 = Force Case to Upper
    var scwDateOutputCase = 0;
...
// -----------------------------------------------------------------------------
//Functions added by Massimo Balestra Aug 14 2013
//------------------------------------------------------------------------------
    ...
    Date.prototype.scwFormat =
        function(scwFormat)
            {var charCount = 0,
                     ...
                     switch (codeChar)
                                {case '^':
                                 ...
                                 case '%':
                                    ...
                                    // the month name.
                                    result += (charCount<3) ? (this.getMonth()+1).toString().scwPadLeft(charCount) : (charCount<4) ? scwArrMonthNames[this.getMonth()] : scwArrFullMonthNames[this.getMonth()];
                                    break;
                                 case '#':
                                    ...
                                    break;
                                 case '{':
                                    // If we find a O, get the ordinal number and display
                                    result += getGetOrdinal(this.getDate()).scwPadLeft(charCount);
                                    break;
                                 case '&':
                                    // If we find a E, get the week day name
                                    // scwArrWeekDays
                                    result += scwArrWeekDays[this.getDay()];
                                    break;
                                 default:
                     ...
                     switch (codeChar)
                                {case 'y': case 'Y':
                                 ...
                                 case 'o': case 'O':
                                    ...
                                    break;
                                 case 'e': case 'E':
                                    // If we find a E, get the week name
                                    // scwArrWeekDays
                                    result += scwArrWeekDays[this.getDay()];
                                    break;
                                 default:
             ...
             result = (scwDateOutputCase == 1) ? result.toUpperCase() : result;
...
// ****************************************************************************
    ...
    function scwShow(scwEle,scwSource)
        ...
        if (!scwSource) {scwSource = window.event;}
         ...
         do 
         {
          if ((typeof scwEle.offsetParent != 'undefined')&&(scwEle.offsetParent != null))
          {
          ...
          {
            scwEle = document.getElementsByTagName('body')[0];
          }
         }
         while (scwEle.tagName!='BODY' && scwEle.tagName!='HTML');
...
// ************************************
    ...
    function scwSetPredefinedDateOutputFormat(tFormat)
    {
      newDateFormat = 1;
      scwDateOutputCase = 0;
      ...
      if (tFormat == 0)
        ...
        { scwDateOutputFormat = '&, %%%% #';scwDateOutputCase = 1; }
      else if (tFormat == 32)
        { scwDateOutputFormat = '&, %%%% #, ^^^^';scwDateOutputCase = 1; }
      else if (tFormat == 33)
        { scwDateOutputFormat = '&, %%%% {';scwDateOutputCase = 1; }
        ...
        { scwDateOutputFormat = '&, the { of %%%%';scwDateOutputCase = 1; }
      else if (tFormat == 36)
        { scwDateOutputFormat = '&, the { of %%%%, ^^^^';scwDateOutputCase = 1; }
      else if (tFormat == 37)
        { scwDateOutputFormat = '%%%% {, ^^^^';scwDateOutputCase = 1; }
      ...
      //alert ("scwDateOutputFormat="+scwDateOutputFormat+" tFormat="+tFormat);
    }
